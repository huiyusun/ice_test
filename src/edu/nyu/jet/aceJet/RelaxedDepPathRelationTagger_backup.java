...
public class RelaxedDepPathRelationTagger_backup {

	final static Logger logger = LoggerFactory.getLogger(RelaxedDepPathRelationTagger_backup.class);
	...
	private static void predictRelation(AceEntityMention m1, AceEntityMention m2, SyntacticRelationSet relations) {
		...
		Event event = new Event("UNK", new String[] { pathRegularizer.regularize(path), m1.entity.type, m2.entity.type });
	...
	public static void main(String[] args) throws IOException {
		...
		if (args.length != 6) {
			System.err.println(RelaxedDepPathRelationTagger_backup.class.getName()
		...
		if (txtFiles.length != outputFiles.length || txtFiles.length != answerFiles.length) {
		...
		if (searchMode) {
			...
			pathRelationExtractor.updateWeights(100, 100, 100);
	...
	static double precision() {
		int responseCount = RelationScorer.correctRelationMentions + RelationScorer.relationMentionTypeErrors
				+ RelationScorer.spuriousRelationMentions;
		int keyCount = RelationScorer.correctRelationMentions + RelationScorer.relationMentionTypeErrors
