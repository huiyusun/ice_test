...
public class RelationBuilder {
	...
	/**
	 * box for relation set: once seed is filled in (with dependency path), invoke Jet.RelationAL.Bootstrap to build a set
	 * of similar patterns to form relation
	 */
	...
	Box relationSetBox() {
		...
		runButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				if (Ice.relations.containsKey(tempName)) {
					JOptionPane.showMessageDialog(Ice.mainFrame, tempName + " relation already exists. Choose another name.",
		...
		saveButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				...
				if (text.length() == 0 || nameField.getText().trim().length() == 0)
					return;
				...
				try {
					...
					JOptionPane.showMessageDialog(Ice.mainFrame, e.getMessage(), "Unable to proceed",
	...
	public void buildRelation(String inName, String inSeed) {
		...
		ActiveLearner bootstrap = new ActiveLearner(
				new SwingProgressMonitor(Ice.mainFrame, "Initializing Relation Builder", "Collecting seeds...", 0, 5));
		RelationBuilderFrame frame = new RelationBuilderFrame("Active Learning relations", RelationBuilder.this, bootstrap,
				null, inSeed);
		...
		RelationBuilderThread builder = new RelationBuilderThread(seed, Ice.selectedCorpus.relationInstanceFileName,
				pathListFileName, RelationBuilder.this, bootstrap, frame, null);
		builder.start();
	}
}
